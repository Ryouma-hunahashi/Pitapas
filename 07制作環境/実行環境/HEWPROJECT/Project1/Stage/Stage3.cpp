...
Stage3::Stage3()
{
	...
	// テクスチャ読み込み
	{
		...
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage3/pitapasu_lake_far.png", &textureBackGround);		// 背景テクスチャ（後景）
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage3/pitapasu_lake_close.png", &textureBackfrontGround);	// 背景テクスチャ(前景)
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage2/pitapasu_cave_ground01.png", &textureFloor);		// 地面テクスチャ
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage2/pitapasu_cave_ground02.png", &textureFloorbottom);		// 地面テクスチャ
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage2/pitapasu_cave_ground03.png", &textureFloormiddle);		// 地面テクスチャ
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage2/pitapasu_cave_ground04.png", &textureFloortop);		// 地面テクスチャ
		...
		CDirectX::Get()->D3D_LoadTexture(L"asset/gimick/rock.png", &textureWoodBox);							// ギミック木箱1マス
		CDirectX::Get()->D3D_LoadTexture(L"asset/gimick/half rock.png", &texturehalfWoodBox);					// ギミック木箱0.5マス
		...
		CDirectX::Get()->D3D_LoadTexture(L"asset/select/Ui_Text_StageName1-3.png", &texturePauseStageName);
	...
	// オブジェクトの実体化
	{
		...
		woodbox[2] = new CWoodBox(vertexBufferPiece, textureWoodBox, LayerNum::PLAY_GROUND);				// 木箱
		woodbox[3] = new CWoodBox(vertexBufferPiece, textureWoodBox, LayerNum::PLAY_GROUND);				// 木箱
		...
		Pieceeffect = new CGameObject(vertexBufferPieceEffect, textureEffectPanel, LayerNum::UI);
	...
	// 初期座標設定
	{
		...
		// 地面画像
		{
			for (int i = 0; i < 6; i++)
			{
				floor[i]->transform->scale = _scaling;
				floor[i]->transform->position = piece_array[3][i]->transform->position;
				floor[i]->collider->transform = floor[i]->transform;
				//floor[i]->transform->position.y += 0.1f;
			}
			floor[6]->transform->position = piece_array[1][6]->transform->position;
			floor[6]->Settexture(textureFloortop);
			floor[6]->transform->scale = _scaling;
			floor[7]->transform->position = piece_array[2][6]->transform->position;
			floor[7]->Settexture(textureFloormiddle);
			floor[7]->transform->scale = _scaling;
			floor[8]->transform->position = piece_array[3][6]->transform->position;
			floor[8]->transform->scale = _scaling;
			floor[8]->Settexture(textureFloorbottom);
		...
		// プレイヤー
		{
			...
			// プレイヤーの当たり判定のサイズ調整
			//player->collider->transform->position = player->transform->position;
		...
		// 木箱（1マスサイズ）
		{
			woodbox[0]->transform->position = piece_array[2][2]->transform->position;
			woodbox[0]->transform->scale = {2.2f,2.2f,1.0f};
			woodbox[0]->transform->position.y += 0.2f;

			woodbox[1]->transform->position = piece_array[2][3]->transform->position;
			woodbox[1]->transform->scale = { 2.2f,2.2f,1.0f };
			woodbox[1]->transform->position.y += 0.2f;

			woodbox[2]->transform->position = piece_array[2][4]->transform->position;
			woodbox[2]->transform->scale = { 2.2f,2.2f,1.0f };
			woodbox[2]->transform->position.y += 0.2f;

			woodbox[3]->transform->position = piece_array[1][3]->transform->position;
			woodbox[3]->transform->scale = { 2.2f,2.2f,1.0f };
			woodbox[3]->transform->position.y += 0.2f;

		}
		// 木箱（0.5マスサイズ）
		{
			halfwoodbox[0]->transform->position = piece_array[2][1]->transform->position;
			...
			halfwoodbox[1]->transform->position = piece_array[2][5]->transform->position;
		...
		// ゴール画像　テストで鍵になっている
		{
			goal->transform->position = piece_array[0][6]->transform->position;
			goal->transform->scale = { 2.3f,2.3f,1.0f };

			goaleffect->anim = new CAnimation(10, 6);
			goaleffect->anim->SetPattern(CAnimation::FX_0_54);
			goaleffect->anim->animSpeed = 0.4f;

			goaleffect->transform->position = piece_array[0][6]->transform->position;
			goaleffect->transform->scale = { 1.1f,1.1f,1.0f };
			goaleffect->transform->position.x += 0.1f;
		}
		// エフェクト
		{
	...
	// 各種可変長配列に格納
	{
		// コライダーをコライダー用可変長配列に格納する
		colliderObj.push_back(woodbox[0]->collider);
		colliderObj.push_back(woodbox[1]->collider);
		colliderObj.push_back(halfwoodbox[0]->collider);
		for (int i = 0; i < 9; i++)
		{
		...
		gimickObj.push_back(woodbox[2]);
		gimickObj.push_back(woodbox[3]);
		//childObj.push_back(woodbox[4]);

		gimickObj.push_back(halfwoodbox[0]);
		gimickObj.push_back(halfwoodbox[1]);
		//childObj.push_back(halfwoodbox[2]);
	}
	// 描画するオブジェクトを配列に格納する
	{
		...
		//drawManager->AddObject(woodbox[2]->collider);		// 木箱（1ピース）コライダー
		...
		//drawManager->AddObject(halfwoodbox[0]->collider);			// 木箱（0.5ピース）
		//drawManager->AddObject(halfwoodbox[1]->collider);			// 木箱（0.5ピース）
		...
		drawManager->AddObject(woodbox[2]);				// 木箱（1ピース）
		drawManager->AddObject(woodbox[3]);				// 木箱（1ピース）
		//drawManager->AddObject(woodbox[4]);				// 木箱（1ピース）
	...
	for (int i = 0; i < PIECE_Y; i++)
	{
		for (int j = 0; j < PIECE_X; j++)
		{
			piece_array[i][j]->isDraw = false; // ピースの配列
			piece_array[i][j]->collider->isDraw = false; // ピースの配列
		}
	}
	player->collider->isDraw = false;

	// 描画しない物をオフにしておく
	//floor[3]->isDraw = false;
	//floor[3]->collider->isDraw = false;
	//floor[5]->isDraw = false;
	//floor[5]->collider->isDraw = false;

	//woodbox->collider->isGravity = true;
	

	drawManager->Update();

	CSoundManager::GetSound()->XA_Play(STAGE3_BGM, 0.4f);
}
...
void Stage3::Update()
{
	...
	else if (pauseSet && !fadeout)	// ポーズ画面
	{
		...
		if (CInput::Get()->GetType() == CInput::HIDType::KeyBoardMouse ? CInput::Get()->isMouseTrigger(CInput::InputType::LB1) : CInput::Get()->isKeyTrigger(CInput::InputType::A))
		{
			...
			else if (pauseCnt == 1)
			{
				...
				CSoundManager::GetSound()->XA_Stop(STAGE3_BGM);
			...
			{
				CSceneManager::get()->ChangeStage(CSceneManager::StageState::STAGE3);
				// ポーズ状態を解除する
				pauseSet = false;
			}
		}
	...
	else
	{
		...
		for (auto&& f : floorObj)
		{
			if (player->collider->transform->Right > floor[6]->transform->Left && player->sideflg && player->transform->Top > floor[6]->transform->Top)
			{
				player->CheckJump_Turn(floor[6]->transform);
				break;
			}
			else if (player->collider->transform->Right > floor[6]->transform->Left && player->sideflg)
			{
				player->CheckJump_Turn(floor[6]->transform);
				break;
			}
			else if (player->collider->transform->Right > floor[7]->transform->Left && player->sideflg)
			{
				player->CheckJump_Turn(floor[7]->transform);
				break;
			}
		...
		// ゴールの当たり判定
		if (player->collider->transform->Right > goal->collider->transform->position.x && player->transform->Top > goal->transform->Bottom)
		{
			resultset = true;
		}
...
void Stage3::ReplaceObj(CPiece* _pieceobj)
{
	for (auto&& f : floorObj)
	{
		...
		if (hitobj)// 中にあったとき
		{
			for (auto&& o : gimickObj)
			{
				...
				else // 格納されていなければ
				{
					...
					//d->transform->position.x += 0.01f;
					// 何も入っていなければUIの所に子オブジェクトを配置する

					//d->transform = previewpiece->transform;
