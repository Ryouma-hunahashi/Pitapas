...
Stage2::Stage2()
{
	...
	// テクスチャ読み込み
	{
		...
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage2/pitapasu_cave_far.png", &textureBackGround);		// 背景テクスチャ（後景）
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage2/pitapasu_cave_close.png", &textureBackfrontGround);	// 背景テクスチャ(前景)
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage2/pitapasu_cave_ground01.png", &textureFloor);		// 地面テクスチャ
		CDirectX::Get()->D3D_LoadTexture(L"asset/stage2/pitapasu_cave_ground02.png", &textureFloormiddle);		// 地面テクスチャ
		...
		CDirectX::Get()->D3D_LoadTexture(L"asset/select/Ui_Text_StageName1-2.png", &texturePauseStageName);
		...
		CDirectX::Get()->D3D_LoadTexture(L"asset/gimick/needle_f.png", &textureThorn);						// テキスト：もどる
	}
	// オブジェクトの実体化
	{
		...
		goaleffect = new CGameObject(vertexBufferEffect, textureEffect, LayerNum::FORE_GROUND);					// ゴールエフェクト
		Pieceeffect = new CGameObject(vertexBufferPieceEffect, textureEffectPanel, LayerNum::UI);	// ピースエフェクト
		fade = new CGameObject(vertexBufferBg, texturefade, LayerNum::BLACK_OUT);								// フェード
		fadeblack = new CGameObject(vertexBufferBg, texturefade, LayerNum::BLACK_OUT);							// 
		mousecursor = new CGameObject(vertexBufferPiece, textureMouseCursor, LayerNum::BLACK_OUT);				// マウスカーソル
	...
	// 初期座標設定
	{
		...
		// 地面画像
		{
			_position = { -6.9f,-3.35f,0.2f };
			for (int i = 0; i < 6; i++)
			{
				floor[i]->transform->scale = { 2.2f,2.2f,1.0f };
				floor[i]->transform->position = _position;
				_position.x += 2.31f;
			}
			floor[3]->transform->position.y -= 2.3f;
			floor[6]->transform->position = piece_array[2][6]->transform->position;
			floor[6]->transform->scale = { 2.2f,2.2f,1.0f };
			floor[6]->Settexture(textureFloortop);
			floor[7]->transform->position = piece_array[3][6]->transform->position;
			floor[7]->transform->scale = { 2.2f,2.2f,1.0f };
		...
		// UIピース置き場画像
		{
			uipiecestock->transform->position = { -6.5f,3.3f, 1.0f };
		...
		// 木箱（1マスサイズ）
		{
			...
			woodbox[1]->transform->position = piece_array[2][4]->transform->position;
		...
		// 木箱（0.5マスサイズ）
		{
			...
			halfwoodbox[0]->collider->transform = halfwoodbox[0]->transform;

		}
		// ゴール画像　テストで鍵になっている
		{
			goal->transform->position = { 7.0f,1.0f,1.0f };
	...
	// 描画するオブジェクトを配列に格納する
	{
		...
		//drawManager->AddObject(halfwoodbox[0]->collider);	// 木箱（0.5ピース）コライダー
		...
		drawManager->AddObject(Pieceeffect);					// エフェクト
	...
	drawManager->Update();
	CSoundManager::GetSound()->XA_Play(STAGE2_BGM, 0.4f);
}
...
Stage2::~Stage2()
{
	...
	delete woodbox[0];
	delete woodbox[1];
	delete halfwoodbox[0];
	delete halfwoodbox[1];
	delete goal;
	delete pazle[0];
	delete pazle[1];
	delete player;
...
void Stage2::Update()
{
	...
	else if (pauseSet && !fadeout)	// ポーズ画面
	{
		...
		if (CInput::Get()->GetType() == CInput::HIDType::KeyBoardMouse ? CInput::Get()->isMouseTrigger(CInput::InputType::LB1) : CInput::Get()->isKeyTrigger(CInput::InputType::A))
		{
			...
			else if (pauseCnt == 1)
			{
				...
				CSoundManager::GetSound()->XA_Stop(STAGE2_BGM);
	...
	else
	{
		...
		for (auto&& f : floorObj)
		{
			if (player->collider->transform->Right > floor[6]->transform->Left && player->sideflg && player->collider->transform->Bottom+0.2f < floor[6]->transform->Top)
			{
...
void Stage2::ReplaceObj(CGameObject* _pieceobj)
{
	for (auto&& f : floorObj)
	{
